...
def calculation(spisok: list): # цикличное выполнение операций по порядку с добавлением в список результата и удаления исходных данных
...
def open_brackets(spisok: list) -> list:
    while '(' in spisok:
        ...
        if dlina == 1:
            spisok.insert(last, temp_spisok[0]) # добавляем в исходный список результат выражения в скобках
